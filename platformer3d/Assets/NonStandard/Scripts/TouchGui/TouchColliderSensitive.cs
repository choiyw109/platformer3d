...
namespace NonStandard.TouchGui
{
	public class TouchColliderSensitive : MonoBehaviour
	{
				...
				if (value == true) {
					if (!_pressed) {
						if (anyTouchTrigger || triggeringCollider.touch.phase == TouchPhase.Began) {
							//Debug.Log("Pressed");
							PressDown(triggeringCollider);
						} else {
							return;
						}
					} else {
						//Debug.Log("Held");
						Hold(triggeringCollider);
					}
				} else if (value == false && _pressed == true) {
					//Debug.Log("Released");
					Release(triggeringCollider);
				}
				_pressed = value;
			}
			get { return _pressed; }
		...
		public virtual void Start() {
			...
			NonStandard.MainTimer.Instance(); // allows colliders to disable on the next frame
